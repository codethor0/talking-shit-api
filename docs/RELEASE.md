...
12. If the release changes non-versioned settings, materialize the prior signed release's exact `wrangler.jsonc` into a temporary `KNOWN_GOOD_CONFIG` and record its source tag and hash. Independently capture the live control-plane baseline from the Cloudflare dashboard or Worker Script Settings API and prove that it matches the expected prior-release state. The signed file is an intended recovery anchor, not proof of current live state. Stop on drift.
13. Stage the known-good Worker at 100 percent and the candidate at 0 percent using `KNOWN_GOOD_CONFIG`, then confirm the deployment state exactly and re-read the live control-plane baseline. The post-stage state must match the pre-stage snapshot.
...
20. After promotion, confirm deployment state is the exact candidate at 100 percent, then require sustained public convergence. Send each probe as a separate request with a unique `X-Probe-Id` header and no-cache request headers; do not add a query string, because the parameterless routes reject one from v0.8.0 (see `docs/security/DEPLOYMENT.md` section 9). At minimum, require 10 consecutive paired observations where `/v1/health` reports the expected `meta.service_version` and `/openapi.json` reports the expected `info.version`. Reset the streak on any mismatch.
